...
class EnergyDiffusionSampler(GroupEnergySampler):
    ...
    @torch.no_grad()
    def forward_diffusion(self, bsize: int, t: int) -> Tuple[Tensor, Tensor]:
        """
    ...
    def estimate_score(self, g: Tensor, x: Tensor, y: Optional[Tensor], t: int) -> Tensor:
        ...
        # forward diffusion
        w, log_modular_w = self.forward_diffusion(self.num_mc, t)
        ...
        energy = self.energy(w_g_inv, x_, y_) / self.temperature #- self.group.log_det_jacobian(w_g_inv, x_)
